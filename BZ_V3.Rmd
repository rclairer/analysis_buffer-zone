...
#structural.data <- filter(data, ID != "AR-378_RB1_2015-09-21_S_T0")
...
#env.species.bcd <- filter(env.species.bcd, ID != "AR-378_RB1_2015-09-21_S_T0")
...
set.seed(318)
#species.data.st.bcd <- species.bcd
...
#library(agricolae)
#nrhsd <- HSD.test(nranova, c("NEAR_DIST_km"))
#nrhsd
...
nrabundtest <- summarySE(nrmetrics, measurevar = "abund", groupvars = c("NEAR_DIST_km", "Structure_Type"))
...
plot.nr.abund + 
  ...
  theme(axis.text=element_text(size=16, colour="black"), 
        ...
        axis.line.y = element_line(color="black", size = 0.5))+
geom_point (colour = "grey") + 
  scale_y_continuous(name="Abundance", limits=c(0, 300), breaks=seq(0, 300, by=50)) +
  scale_x_continuous(name = "Distance (km)", limits=c(0, 1.2), breaks=seq(0, 1.2,by=0.2)) + 
  #geom_smooth(data = less40, colour = "black", fullrange = FALSE, method = "lm")+
  #geom_smooth(data = greater70, colour = "black", fullrange = FALSE, method = "lm")+
  geom_smooth(colour = "black",fullrange = FALSE, method = "lm")+
  #testing+
  ggtitle("Abundance on AR by Distance from NR")

nrbiomass <- join(nr, fishbiomass, by = c ("Site", "Subsite"))
nrbiomassanova <- lm(Biomass ~ NEAR_DIST_km, data = nrbiomass)
anova(nbiomassdanova)
summary(nrbiomassanova)
...
NRin <- filter(all.species.f, IN_Subsite == "NR1"| IN_Subsite == "NR2")
...
ggplot(test, aes(x=as.factor(Transect_Number), y=Abundance)) +
  ...
  scale_y_continuous(name="Average Fish Abundance (ind./m^2)", 
                     limits=c(0, 500), 
...
attach(fishabundance)
qqnorm(Abundance)
qqline(Abundance)
shapiro.test(Abundance)

y <- Abundance
G <- Transect_Number

# Bartlett Test of Homogeneity of Variances
bartlett.test(y~G, data=fishabundance)

# Figner-Killeen Test of Homogeneity of Variances
fligner.test(y~G, data=fishabundance)
...
everything <- lm(Abundance ~ Transect_Number * Sampling_Period * Site * Reef_Type * DRR * Latitude_DD * Temp_min * avg_dep * Structure_Type * Distance_To_Nearest_Land_ft* ver_rel_m, data = fishabundance)
anova(everything)
summary(everything)
