...
# new ones
join4 <- cbind(join3[,1:8], join3[,80:103], join3[,9:79]) # species
...
join5 <- join4[,-c(9, 13)] # FOR SPECIES
...
env.end<- 30 #FOR SPECEIS
sp.start<-31 #FOR SPECIES
...
plot.all + geom_point (aes(colour = Sampling_Period)) +theme_classic () +  scale_y_continuous(name="Bray Curtis Dissimilarity", limits=c(0, 1), breaks=seq(0, 1, by=0.1)) + scale_x_continuous(name = "Distance (km)", limits=c(0, 120), breaks=seq(0, 120,by=20)) + geom_smooth(aes(colour = Sampling_Period))
...
p<-filter(site378.fall, BCD == max(site378.fall$BCD))
q <-filter(site378.spring, BCD == max(site378.spring$BCD)) 
r <- filter(site378.winter, BCD == max(site378.winter$BCD))

findmaxbcd <- rbind(a, b, c, d, e, f, g, h, i, j,k,l,m,n,o,p,q,r)
avgdist <- ave(findmaxbcd$NEAR_DIST_km)

s <- filter(fall.MHC.f, BCD == max(fall.MHC.f$BCD))
#t <- filter(fall.wil.f, BCD == max(fall.wil.f$BCD)) #max BCD is 1
...
```{r ggplot2}
library(ggplot2)
fishabundance <- mutate(data, Abundance = rowSums(data[,31:ncol(data)]))
...
ggplot(test, aes(x=as.factor(Transect_Number), y=Abundance)) +
  ...
  ggtitle("Fish Abundance per Transect Type")+
annotate("text", x = 1, y = 440, label = "a", size = 5)+
annotate("text", x = 2, y = 174, label = "b", size = 5)+
annotate("text", x = 3, y = 63, label = "b", size = 5)+
annotate("text", x = 4, y = 31, label = "b", size = 5)
...
ann_text <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(850,450,150,50),lab = c("a", "ab","b","b"),Sampling_Period = factor("Fall",levels = c("Fall","Winter","Spring")))
ann_text_winter <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(510,230,90,40),lab = c("ab", "b","b","b"),Sampling_Period = factor("Winter",levels = c("Fall","Winter","Spring")))
ann_text_spring <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(120,50,40,40),lab = c("b", "b","b","b"),Sampling_Period = factor("Spring",levels = c("Fall","Winter","Spring")))
...
ggplot(test1, aes(x=as.factor(Transect_Number), y=Abundance)) +
  ...
  theme(axis.text=element_text(size=16, colour="black"), 
        ...
        axis.line.y = element_line(color="black", size = 0.5))+ 
  scale_x_discrete(name = "Transect Type", labels = c("0" = "Structural", "1" = "0-30 m", "2" = "30-60 m", "3" = "60-90 m"))+
ggtitle("Fish Abundance per Transect Type per Season")+
...
annotate("text", x = 2, y = 170, label = "ab", size = 5)+
annotate("text", x = 3, y = 50, label = "b", size = 5)
####################################################################################
...
ggplot(test2, aes(x=as.factor(Transect_Number), y=Abundance, fill=Reef_Type)) +
  ...
  scale_x_discrete(name = "Transect Type", labels = c("0" = "Structural", "1" = "0-30 m", "2" = "30-60 m", "3" = "60-90 m"))
#***************************************************8
ann_text_art <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(500,200,80,20),lab = c("a", "ab","b","b"),Reef_Type = factor("Artificial",levels = c("Artificial","Natural")))
ann_text_nat <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(330,80,30,45),lab = c("ab", "ab","b","b"),Reef_Type = factor("Natural",levels = c("Artificial","Natural")))
...
ggplot(test2, aes(x=as.factor(Transect_Number), y=Abundance)) +
  ...
  theme(axis.text=element_text(size=16, colour="black"), 
        ...
        legend.position="bottom") + 
  scale_x_discrete(name = "Transect Type", labels = c("0" = "Structural", "1" = "0-30 m", "2" = "30-60 m", "3" = "60-90 m"))+
ggtitle("Fish Abundance per Transect Type per Reef Type")+
...
annotate("text", x = 1, y = 155, label = "a", size = 5)+
annotate("text", x = 2, y = 90, label = "a", size = 5)
...
ann_text_330 <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(500,200,80,20),lab = c("a", "ab","b","b"),Site = factor("370",levels = c("330","345","364","370","372","378")))

ann_text_330 <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(500,200,80,20),lab = c("a", "ab","b","b"),Site = factor("372",levels = c("330","345","364","370","372","378")))
ann_text_330 <- data.frame(Transect_Number = c(0,1,2,3),Abundance = c(500,200,80,20),lab = c("a", "ab","b","b"),Site = factor("378",levels = c("330","345","364","370","372","378")))
...
annotate("text", x = 1, y = 530, label = "a", size = 5)+
annotate("text", x = 2, y = 220, label = "ab", size = 5)+
annotate("text", x = 3, y = 190, label = "b", size = 5)+
annotate("text", x = 4, y = 70, label = "b", size = 5)+
annotate("text", x = 5, y = 170, label = "b", size = 5)+
annotate("text", x = 6, y = 50, label = "b", size = 5)
...
library(agricolae)
hsd11.reef <- HSD.test(mod11, c("Reef_Type"))
hsd11.reef
