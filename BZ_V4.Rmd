...
#join4 <- cbind(join3[,1:6], join3[,80:109], join3[,7:79]) # species
...
#joinbiom4 <- cbind(joinbiom3[,1:6], joinbiom3[,80:109], joinbiom3[,7:79])
...
trythis <- summarySE(comm_mets, measurevar = "abund", groupvars = c("Site", "Subsite", "Date", "Transect_Number"))
...
#geom_point (colour = "grey") + 
 # geom_point (aes(colour = GIS_FID)) + 
  ...
  #geom_line(aes(group = interaction(Site, Subsite, Date), linetype = Sampling_Period))+
    geom_line(aes(group = interaction(Site, Subsite, Date)))+
    # geom_smooth (aes(colour = GIS_FID)) + 
#geom_errorbar(aes(ymin=abund-se, ymax=abund+se), width=.05, position=position_dodge(0.9))+
  scale_y_continuous(name="Average Fish Abundance", limits=c(0, 35), breaks=seq(0, 35, by=5)) +
