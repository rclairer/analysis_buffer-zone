...
#REMOVE SAILBOAT AND UNKNOWN STRUCUTRE

rad <- filter(fishabundance, Transect_Type == "Radiating")

radstruc <- filter(rad, Structure_Present == "Yes")
radstruc$Radiating_Structure <- revalue(radstruc$Radiating_Structure, c("Structure" = "Unknown Structure"))
radstruc$Radiating_Structure <- revalue(radstruc$Radiating_Structure, c("Reef ball" = "Reef Ball"))
#radstruc$Radiating_Structure <- as.character(radstruc$Radiating_Structure)
radstruc[14,10] = "Unknown Structure"

#radstruc$Structure_Present <- as.factor(radstruc$Structure_Present)
#REMOVE SAILBOAT AND UNKNOWN STRUCUTRE

radstruc <- filter(radstruc, Radiating_Structure != "Sailboat")
radstruc <- filter(radstruc, Radiating_Structure != "Unknown Structure")
#radstruc$Radiating_Structure <- ifelse(nchar(radstruc$Radiating_Structure)==0, "Unknown Structure", radstruc$Radiating_Structure)

#by transect

radstrucstats <- summarySE(radstruc, measurevar = "Abundance", groupvars = c("Transect_Number", "Radiating_Structure"))

radstrucstatsmore <- summarySE(radstruc, measurevar = "Abundance", groupvars = c("Radiating_Structure"))
#############################################################
#not as helpful
adjacent_type <- ggplot(radstrucstats, aes(as.factor(Transect_Number), Abundance, fill = Radiating_Structure)) + geom_bar(stat = "identity", position = "dodge")

adjacent_type + theme_classic()
#########################################################3
#by type
adjacent_type <- ggplot(radstrucstatsmore, aes(Radiating_Structure, Abundance)) + geom_bar(stat = "identity", position = "dodge")
...
adjacent_type + theme_classic()+
  ...
  scale_x_discrete(name = "Structure Type")+
ggtitle("Fish Abundance per Structure Type")+
annotate("text", x = 1, y = 15, label = "a", size = 5)+
annotate("text", x = 2, y = 63, label = "a", size = 5)+
annotate("text", x = 3, y = 110, label = "a", size = 5)+
annotate("text", x = 4, y = 36, label = "a", size = 5)+
annotate("text", x = 5, y = 18, label = "a", size = 5)
...
pair
###################################
fishabundance$Structure_Present <- as.factor(fishabundance$Structure_Present)
...
library(agricolae)
hsd2.samp <- HSD.test(mod2, c("Structure_Present", "Transect_Number"))
hsd2.samp
...
############################################
mod3 <- lm(Abundance ~ Radiating_Structure, data = radstruc)
anova(mod3)
summary(mod3)

hsd2.radstruc <- HSD.test(mod3, "Radiating_Structure")
hsd2.radstruc
```
...
permanova_trans<-adonis(species.data ~ Transect_Number, env_data, permutations = 1000, method = "bray")
permanova_trans
summary(permanova_trans)


env_data$Structure_Present <- as.factor(env_data$Structure_Present)
permanova_struc<-adonis(species.data ~ Structure_Present, env_data, permutations = 1000, method = "bray")
permanova_struc
summary(permanova_struc)


permanova_trans_struc<-adonis(species.data ~ Transect_Number * Structure_Present, env_data, permutations = 1000, method = "bray")
permanova_trans_struc
summary(permanova_trans_struc)
...
groups_structure <- paste(as.character(env_data$Structure_Present))
groups_structure <- as.factor(groups_structure)
simp_structure <- simper(species.data, groups_structure)
simp_structure
summary(simp_structure, ordered = TRUE)
...
#filter for blank in group struct!!!!!
...
indval_season <- multipatt(species.data, groups_season, control = how(nperm = 999))
summary(indval_season)

grouops_reef <- env_data$Reef_Type
  ...
  #geom_text(data=species.wa,
    ...
    theme(axis.text=element_text(size=16), 
     ...
     scale_colour_manual(name = "Distance from Reef", labels = c("0" = "Reef", "1" = "0-30 m", "2" = "30-60 m", "3" = "60-90 m"),
      ...
      #scale_colour_manual(name = "Distance from Reef", 
                    #   values = c("0" = "indianred2", "1" = "seagreen", "2" = "chartreuse", "3"= "darkturquoise"), lables = c("0" = "Reef", "1" = "0-30 m", "2" = "30-60 m", "3" = "60-90 m"))+
      ggtitle(title)
...
#  species.nms$Structure_Present <- as.factor(species.nms$Structure_Present)
  ...
  #  coord_equal()+ # axes sizes equal
   # theme_bw()+ # basic theme
  #  geom_point(data = NMDS, 
 #             aes(x = MDS1, y = MDS2, color = species.nms$Structure_Present, shape = #species.nms$Transect_Number), size = 5) +
  #stat_ellipse(data = NMDS, 
          #      aes(x=MDS1,y=MDS2,colour=species.nms$Structure_Present, shape = species.nms$Transect_Number),
            #     level = 0.50, linetype=2, size =1)+
  #geom_text(data=species.wa,
             # aes(x=species.wa[,2],y=species.wa[,3],label=species_code),
             # alpha=0.9, # adds weighted averages for species
  ...
  #  theme(axis.text=element_text(size=16), 
      ...
      #scale_colour_manual(name = "Structure", 
     #                  values = c("Yes" = "indianred2", "No" = "darkturquoise"))+
    #scale_shape_manual(name = "Distance from Structure", labels = c("0" = "Structure", "1" = "0-30 m", "2" = "30-60 m", "3" = "60-90 m"))+
   #   ggtitle(title)
