...
library(dplyr)
library(lubridate)
fish_data <- get_fish_data (file_data = "Fish/3_Clean_Data/fish_data-clean.csv",
               size = "Abundance")
fish_data$Date <- mdy(paste(fish_data$Date))
...
fish_abund_matrix <- calc_fish_abund_matrix (data = fish_subset, 
                                      tax_level = "Species_Code",
                                      trans_level = "avgd")
fish_biom_matrix <- calc_fish_biom_matrix (data = fish_subset, 
                                           tax_level = "Species_Code",
#                                    tax_level = "Species_Code",
                                    trans_level = "avgd")
...
ben_data <- get_ben_data (file_data = "Benthos/3_Clean_Data/benthic_data-clean.csv" )
library(reshape2)
ben_long <- melt(ben_data, 
                  id.vars=names(ben_data[,1:9]), 
                  measure.vars = names(ben_data[,10:116]), 
                  variable.name= "species", 
                  value.name = "cover")
ben_meta <- combine_data_meta (data = ben_long, 
                               file_meta = "Metadata/Benthos/Benthic_Codes.csv", 
                               merge_by = "species")
#ben_subset <- subset_data (data = ben_meta,
#                             sub_cat = "phlya",
#                             sub_val = "CNIDARIA")
ben_cov <- calc_ben_cover (data = ben_meta,
                           tax_level = "phyla", # for taxonomic level of interest 
                           trans_level = "avgd", # for either indiv transects, avgd transects, or quads
                           start = 4) # indiv trans = 5, avgd trans = 4, quad = 6
ben <- ben_cov
ben$Date <- mdy(paste(ben$Date))


### COMPLEXITY DATA ### -------------------------------------------------------
comp_data<-read.table("Complexity/3_Clean_Data/complexity.csv", header=T, sep=',')
comp <- calc_comp_metrics(data = comp_data, trans_level = "avgd")
comp$Date <- mdy(paste(comp$Date))
rm(comp_data)

### SEDIMENT DATA ### ---------------------------------------------------------
sed_data<-read.table("Sediment/3_Clean_Data/sediment_data-clean.csv", header=T, sep=',')
sed <- calc_sed_metrics (data = sed_data, trans_level = "avgd")
sed$Date <- ymd(paste(sed$Date))
rm(sed_data)

### TEMPEATURE DATA ### -------------------------------------------------------
temp_data<-read.table("Temperature/3_Clean_Data/temp_data-clean.csv", header=T, sep=',')
temp <- calc_temp_metrics (data = temp_data, trans_level = "avgd")
temp$Date <- ymd(paste(temp$Date))
rm(temp_data)

### SITES METADATA ### ------------------------------------------------------------
sites <-read.table("Metadata/Sites/Sites.csv", header=T, sep=',')
...
#data  <-inner_join(join.3, fish, by = c("Site", "Date", "Sample"))
...
biom_data<-merge(merge.3, fish_biom_matrix)
...
# Plots
plot <- plot_nmds_reef_type(species.nms, species.wa)
...
```{r CCA}
library(vegan)
mod.1<-cca ( species.data ~ avg_dep + ver_rel, env_data)
...
#env.data.pca<-env.data[,c(16,28, 26, 33)] # select variables of interest
## MAKE CHOICE - for artificial reefs exclude sediment
env.data.pca<-env_data[,c(15, 18, 24, 25)] # select variables of interest for NATURAL
#env.data.pca<-env_data[,c(14, 17, 24)] # for artificial
...
cor.test(env.data.pca$sed_stdv,env.data.pca$DRR)
...
detach("package:dplyr", unload=TRUE)
library(plyr)
env.data.pca<-rename(env.data.pca, c("sed_stdv"="Sediment", "avg_dep"="Depth", "DRR"="Complexity", "temp_avg"="Temperature"))

# a table of correlations between PCs and the original variables:
library(ecodist)
